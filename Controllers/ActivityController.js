...
exports.seeStudentActivity = async (req,res,next) =>{
    ...
    switch(turn.state){
        ...
        case 3:
            ...
            const sprints = await Sprint.findOne({through: {attributes: ['objetive']},include: [{model: Team, where: {id: team.id}, required: true}, {model: HU, required: true}]});
            instructions = representScenarioPlan(role, view, sprints?.objetive);
            views = allowedViewsPlan[role];
            res.render('activity/Lecture.ejs', {turn, state, instructions: instructions[0], views, view, kit, Hus, poWrite: instructions[2], devWrite: instructions[1], SMWrite: instructions[3], HuSprints: sprints?.HUs, objetive: sprints?.objetive});
        ...
        case 4:
            ...
            const HuSprint = await Sprint.findOne({where: {id: sprintsE.id},through: {attributes: ['objetive']},include: [{model: HU, required:true, include: {model:Result, where: {SprintId: sprintsE.id},required: false}}]});
            instructions = representScenarioSprint(role);
            views = allowedViewsEjecution[role];
            res.render('activity/Lecture.ejs', {turn, state, instructions: instructions[0], views, view, kit, Hus, poWrite: instructions[2], devWrite: instructions[1], SMWrite: instructions[3], HuSprints: HuSprint.HUs, objetive: HuSprint?.objetive});
        ...
        case 5:
            ...
            res.render('activity/Lecture.ejs', {turn, state, instructions: instructions[0], views, view, kit,Hus, poWrite: instructions[2], devWrite: instructions[1], SMWrite: instructions[3], HuSprints: HuSprints.HUs, objetive: HuSprints?.objetive});
...
function representScenarioPlan(role, view, objetive){
    let temp;
    switch(role){
...
exports.addObjetive = async(req,res)=>{
    try{
        const {objetive} = req.body;
        const turn = await Turn.findOne({where: {id: req.params.idTurno}, include: {model: Team, required: true, include: {model: User, where: {id: req.user.id}, required: true}}});
        if(!turn){
            return res.status(404).send();
        }
...
exports.addImprovement = async (req,res,next)=>{
    ...
    try{
        ...
        sprint.burdownChart = burdownImage.path;
        await sprint.save();
        await result.save();
        return res.status(200).send();
    }catch(err){
